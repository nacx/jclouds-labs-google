...
public class NetworkToSecurityGroupTest {
   ...
   public void testApply() {
      ...
      final ListPage<Firewall> page = ListPage.<Firewall> builder().kind(Kind.FIREWALL_LIST)
            .items(ImmutableSet.of(FirewallToIpPermissionTest.fwForTest())).build();

      ListPage<Firewall> listPage = new ListPage<Firewall>(page,
            new Function<ListPage<Firewall>, PagedIterable<Firewall>>() {
               @Override
               public PagedIterable<Firewall> apply(ListPage<Firewall> input) {
